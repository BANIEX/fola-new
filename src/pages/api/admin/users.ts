...
export default async function handler(req: NextApiRequest, res: NextApiResponse<IUser | { status: 'error' | 'valid', description: string, [x: string]: any }>) {
    ...
    try {
        ...
        const users = await db.collection<IProduct>("users").find({}).limit(limit).skip(skip).toArray();
        res.status(200).json({ status: 'valid', description: 'Users found', items: users, slice: [skip, limit] });
    } catch (e) {
