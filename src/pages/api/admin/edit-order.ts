...
export default async function handler(req: NextApiRequest, res: NextApiResponse<IUser | { status: 'error' | 'valid', description: string, [x: string]: any }>) {
    ...
    const body  = req.body;
    if (!body.id) return res.status(400).json({ status: 'error', description: 'No order-id attached' });
    try {
        ...
        if(body.edit) await db.collection<IOrder>('orders').updateOne({id: body.id}, {$set: {status: body.status}});
        if(body.delete) await db.collection<IOrder>("orders").deleteOne({id: body.id});
        res.status(200).json({ status: 'valid', description: 'Order mutated' });
    } catch (e) {
